...
plot_volcano(
  data = volcano_dat$data, 
  anotate_genes = top_genes,
  plot_colors = c("red", "grey", "blue")
)
```

```{r}
gene_label_type = 3 # label top n genes 
num_genes = 10 
sort_type = 3 # distance from the origin 
min_lfc = 3 
min_adjp = 10 
...
if (gene_label_type == 1) {
  genes <- NULL
} else if (gene_label_type == 2) {
  req(input$vol_genes)
  genes <- input$vol_genes 
} else if (gene_label_type == 3) {
  if (sort_type == 1) {
    sorted <- data |>
...
} else if (gene_label_type == 4) {
  sorted <- data |> 
    dplyr::filter(abs(Fold) >= min_lfc & -log10(FDR) > min_adjp)
  genes <- rownames(sorted)
}
```
...
```{r}
top_genes <- volcano_dat$anotate_genes %>% 
  mutate(
    genes = rownames(.), 
    abs_fold = abs(Fold), 
    log_fdr = -log10(FDR),
    distance = sqrt((Fold^2) + (log_fdr^2))
  ) 

View(top_genes) 

top_lfc <- top_genes %>%
  arrange(desc(abs_fold)) %>%
  slice(1:10) %>%
  pull(genes) 
...
plot_volcano(
  data = volcano_dat$data, 
  anotate_genes = top_lfc,
  plot_colors = c("red", "grey", "blue")
)

plot_volcano(
  data = volcano_dat$data, 
  anotate_genes = top_fdr,
  plot_colors = c("red", "grey", "blue")
)

plot_volcano(
  data = volcano_dat$data, 
  anotate_genes = top_dist,
  plot_colors = c("red", "grey", "blue")
)
```
...
x$plot + 
  geom_text(
    data = my_list, 
    aes(x = Fold, y = -log10(FDR), label = rownames(my_list)), 
    nudge_x = 0.25, 
    nudge_y = 0.25, 
    check_overlap = T, 
  )
...
gene_list <- rownames(volcano_dat$anotate_genes)[1:4]
