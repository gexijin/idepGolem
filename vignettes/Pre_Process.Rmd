...
`3`                          Fold-changes with corrected p-values from CuffDiff or any other program
`4`                          Fold-changes only (no p-values or FDR columns available)
...
---------------------------  --------------------------------------------------
`1`                          edgeR: log-CPM with prior.count (library-size adjusted pseudocount) (Visit https://www.rdocumentation.org/packages/SparkR/versions/2.1.2/topics/log2)
...
##### **Fold-change summary inputs**

For fold-change summary statistics, there is only one additional parameter
to provide an input for, `no_fdr`. This is a logical `TRUE/FALSE` to specify
whether the data contains corrected p-values or not. Set `data_file_format = 3`
with `no_fdr = FALSE` when both log2 fold-changes and adjusted p-values are
provided, or `data_file_format = 4` with `no_fdr = TRUE` when only fold-changes
are available. All other parameters can be set to `NULL`.
