...
if (params$data_file_format == 1) {
  ...
  cat("Total counts plot requires raw count data (integer read counts). ",
      "Your data was loaded as a normalized/FPKM/TPM expression matrix, which does not have ",
      "meaningful total-count sums per sample.\n")
}
```
...
} else if (params$data_file_format != 1) {
  cat("Read counts table not shown: filtering statistics are only meaningful for raw count data. ",
      "Your data was loaded as a normalized/FPKM/TPM expression matrix.\n")
} else if (is.null(processed_data$raw_counts)) {
  cat("Read counts table not shown: raw counts were not available after pre-processing.\n")
}
```
...
```{r, results='asis'}
if (!is.null(params$all_gene_info) && "ensembl_gene_id" %in% colnames(params$all_gene_info)) {
...
} else if (is.null(params$all_gene_info)) {
  cat("Gene biotype plot not shown: gene annotation data is unavailable. ",
      "This plot requires gene IDs that iDEP can map to Ensembl records.\n")
} else {
  cat("Gene biotype plot not shown: the gene annotation table does not contain an `ensembl_gene_id` column. ",
      "iDEP was unable to map your gene IDs to Ensembl records.\n")
}
```
...
```{r qc_gene_type, fig.height=6, fig.width=10, results='asis'}
if (params$data_file_format == 1 && !is.null(params$all_gene_info) && "ensembl_gene_id" %in% colnames(params$all_gene_info)) {
...
} else if (params$data_file_format != 1 && (is.null(params$all_gene_info) || !"ensembl_gene_id" %in% colnames(params$all_gene_info))) {
  cat("rRNA/gene type QC plot requires both raw count data and ",
      "Ensembl-mappable gene IDs. Your data is normalized and gene IDs could not be mapped to Ensembl.\n")
} else if (params$data_file_format != 1) {
  cat("rRNA/gene type QC plot requires raw count data (integer read counts). ",
      "Proportions of rRNA and other biotypes are only interpretable from un-normalized counts.\n")
} else {
  cat("rRNA/gene type QC plot not shown: gene IDs could not be mapped to Ensembl records. ",
      "Provide Ensembl IDs or standard gene symbols.\n")
}
```

<br>

```{r qc_chr_counts, fig.height=12, fig.width=10, results='asis'}
if (params$data_file_format == 1 && !is.null(params$all_gene_info) && "ensembl_gene_id" %in% colnames(params$all_gene_info)) {
  ...
  cat("Chromosome raw counts plot requires both raw count data and ",
      "Ensembl-mappable gene IDs for chromosomal assignment. Neither condition is met.\n")
} else if (params$data_file_format != 1) {
  cat("Chromosome raw counts plot requires raw count data. ",
      "Per-chromosome read distributions are not meaningful for normalized expression values.\n")
} else {
  cat("Chromosome raw counts plot not shown: gene IDs could not be mapped to Ensembl records.",
      "Provide Ensembl IDs or standard gene symbols.\n")
}
```
...
```{r qc_chr_normalized, fig.height=12, fig.width=10, results='asis'}
if (!is.null(processed_data$data) && !is.null(params$all_gene_info) && "ensembl_gene_id" %in% colnames(params$all_gene_info)) {
...
} else if (is.null(processed_data$data)) {
  cat("Chromosome normalized expression plot not shown: processed expression data is unavailable.\n")
} else {
  cat("Chromosome normalized expression plot not shown: gene IDs could not be mapped to Ensembl records",
      "Provide Ensembl IDs or standard gene symbols.\n")
}
```
...
if (length(marker_payloads) == 0) {
  if (is.null(params$all_gene_info) || !"ensembl_gene_id" %in% colnames(params$all_gene_info)) {
    cat("Marker gene plots not shown: gene IDs could not be mapped to Ensembl records. ",
        "Provide Ensembl IDs or standard gene symbols.\n")
  } else {
    cat("Marker gene plots not shown: no marker genes from the reference sets were detected in this dataset.\n")
  }
} else {
...
## To Reproduce This Report Locally

1. Install the latest `idepGolem` package (once per machine). All the functions and the Rmarkdown code are downloaded with the package. 
...
2. Download the RData file from the Prep tab. 

3. Load the downloaded `.RData` workspace and render this report directly in R or RStudio:
...
The resulting HTML will reproduce what you see in the Prep tab, using the saved `report_params` list bundled inside every `.RData` download.
