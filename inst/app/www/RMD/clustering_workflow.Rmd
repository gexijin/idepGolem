...
format_enrichment_for_report <- function(enrichment_list) {
  ...
  if ("Grp." %in% colnames(res)) {
    res <- res[, c("Grp.", setdiff(colnames(res), "Grp.")), drop = FALSE]
  }
...
```{r cluster_enrichment_table, echo=FALSE, results='asis'}
should_show <- FALSE
heading <- NULL

if (params$cluster_meth == 1 && !is.null(params$submap_data)) {
  should_show <- TRUE
  heading <- "## Pathway Enrichment for Selected Region"
} else if (params$cluster_meth == 2) {
  should_show <- TRUE
  heading <- "## Pathway Enrichment for k-Means Clusters"
}

if (isTRUE(params$cluster_enrichment_enabled) &&
    should_show &&
    !is.null(params$cluster_enrichment_results)) {

  formatted_table <- format_enrichment_for_report(
    params$cluster_enrichment_results
  )

  if (!is.null(formatted_table) && nrow(formatted_table) > 0) {
    if (!is.null(heading)) {
      cat(heading, "\n\n")
    }
    if (requireNamespace("kableExtra", quietly = TRUE)) {
      table_html <- kableExtra::kbl(
        formatted_table,
        format = "html",
        escape = FALSE,
        align = rep("l", ncol(formatted_table))
      ) |>
...
## Re-running This Report Locally

1. Install the latest `idepGolem` package (once per machine):
...
2. Download the `.RData` file from the Cluster tab, load it, and render this report from your working directory:
...
The resulting HTML will match the clustering report generated inside iDEP because it reuses the saved `cluster_report_params` list bundled with every Cluster `.RData` download.
